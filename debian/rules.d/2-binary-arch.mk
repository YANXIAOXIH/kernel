...
$(stampdir)/stamp-prepare-tree-%: $(archconfdir)/%_defconfig debian/scripts/fix-filenames
	...
	cat $(wordlist 1,3,$^) | sed -e 's/.*CONFIG_VERSION_SIGNATURE.*/CONFIG_VERSION_SIGNATURE="Kylin $(release)-$(revision)-$* $(raw_kernelversion)"/' > $(builddir)/build-$*/.config
	find $(builddir)/build-$* -name "*.ko" | xargs rm -f
	$(build_cd) $(kmake) $(build_O) -j1 olddefconfig syncconfig prepare scripts
	touch $@
...
ifneq ($(skipdbg),true)
	...
	# Add UTS_KYLINOS_RELEASE_ABI since UTS_RELEASE is difficult to parse.
	echo "#define UTS_KYLINOS_RELEASE_ABI $(abinum)" >> $(hdrdir)/include/generated/utsrelease.h
