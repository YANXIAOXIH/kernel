...
# image, or rebuild the entire set of packages using custom patches
# or configs.
AUTOBUILD=
...
Arch=$(shell echo $(arch) | sed -e s/i.86/x86/ -e s/x86_64/x86/ -e s/amd64/x86/ -e s/aarch64.*/arm64/ -e s/mips64el/mips/)
...
archconfdir	:= $(CURDIR)/arch/$(Arch)/configs
...
linux_selftests = breakpoints cpu-hotplug efivarfs memfd memory-hotplug mount net ptrace seccomp timers powerpc user ftrace
