			    ...
			    const struct v4l2_rect *compose)
{
	...
	if (CFG_CHECK(MT8195, p_id)) {
		...
		hdr_top_mask = 0x2F0F0001;
	} else if (CFG_CHECK(MT8188, p_id)) {
		reg = CFG_COMP(MT8188, ctx->param, hdr.top);
		hdr_top_mask = BIT(29) | BIT(28);
	}
			     ...
			     struct mdp_cmdq_cmd *cmd, u32 index)
{
	...
	if (CFG_CHECK(MT8195, p_id)) {
		...
		hdr_hist_addr_mask = BIT(9) | BIT(8);
	} else if (CFG_CHECK(MT8188, p_id)) {
		reg = CFG_COMP(MT8188, ctx->param, hdr.subfrms[index].hist_addr);
		hdr_hist_addr_mask = BIT(9);
	}
	MM_REG_WRITE(cmd, subsys_id, base, MDP_HDR_HIST_ADDR, reg, hdr_hist_addr_mask);
