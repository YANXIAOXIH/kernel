...
static int panel_lvds_unprepare(struct drm_panel *panel)
{
	...
	gpiod_set_value(lvds->pwr_en, GPIO_OUT_ZERO);
	mdelay(5);
...
static int panel_lvds_prepare(struct drm_panel *panel)
{
	...
	gpiod_set_value(lvds->pwr_en, GPIO_OUT_ONE);
	mdelay(5);
...
static int panel_lvds_probe(struct platform_device *pdev)
{
	...
	lvds->pwr_en = devm_gpiod_get(&pdev->dev, "pwr", GPIOD_OUT_HIGH);
	if (IS_ERR(lvds->pwr_en)) {
		dev_err(&pdev->dev, "cannot get pwr_en-gpios %ld\n", PTR_ERR(lvds->pwr_en));
		return PTR_ERR(lvds->pwr_en);
	}
