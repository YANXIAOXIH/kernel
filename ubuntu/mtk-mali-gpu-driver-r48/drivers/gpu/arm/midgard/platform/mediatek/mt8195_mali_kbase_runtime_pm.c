...
static int pm_domain_init(struct kbase_device *kbdev)
{
	 ...
	 */
	if (num_domains < 2 && mfg->num_pm_domains < 2)
		return 0;
	...
	if (num_domains != mfg->num_pm_domains) {
		dev_err(kbdev->dev,
			"Incorrect number of power domains: %d provided, %d needed\n",
			num_domains, mfg->num_pm_domains);
		return -EINVAL;
	}
	...
	for (i = 0; i < num_domains; i++) {
		mfg->pm_domain_devs[i] =
			dev_pm_domain_attach_by_name(kbdev->dev,
					pd_names[i]);
		if (IS_ERR_OR_NULL(mfg->pm_domain_devs[i])) {
			err = PTR_ERR(mfg->pm_domain_devs[i]) ? : -ENODATA;
			mfg->pm_domain_devs[i] = NULL;
			if (err == -EPROBE_DEFER) {
				dev_dbg(kbdev->dev,
...
static int pm_callback_power_on(struct kbase_device *kbdev)
{
	...
	for (p_idx = 0; p_idx < mfg->num_pm_domains; p_idx++) {
		error = pm_runtime_get_sync(mfg->pm_domain_devs[p_idx]);
		if (error < 0) {
			dev_err(kbdev->dev,
...
pm_err:
	if (p_idx >= mfg->num_pm_domains)
		p_idx = mfg->num_pm_domains - 1;
	for (; p_idx >= 0; p_idx--) {
		pm_runtime_mark_last_busy(mfg->pm_domain_devs[p_idx]);
		err = pm_runtime_put_autosuspend(mfg->pm_domain_devs[p_idx]);
		if (err < 0)
			dev_err(kbdev->dev,
...
static void pm_callback_power_off(struct kbase_device *kbdev)
{
	...
	for (i = mfg->num_pm_domains - 1; i >= 0; i--) {
		pm_runtime_mark_last_busy(mfg->pm_domain_devs[i]);
		error = pm_runtime_put_autosuspend(mfg->pm_domain_devs[i]);
		if (error < 0)
			dev_err(kbdev->dev,
				"Power off core %d failed (err: %d)\n",
				i+1, error);
...
static int mali_mfgsys_init(struct kbase_device *kbdev, struct mfg_base *mfg)
{
	...
	mfg->num_pm_domains = GPU_CORE_NUM;
...
static int platform_init(struct kbase_device *kbdev)
{
	...
	for (i = 0; i < mfg->num_pm_domains; i++) {
		pm_runtime_set_autosuspend_delay(mfg->pm_domain_devs[i],
						AUTO_SUSPEND_DELAY);
		pm_runtime_use_autosuspend(mfg->pm_domain_devs[i]);
	}
