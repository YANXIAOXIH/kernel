...
camera_querycap(struct file *file, void *fh, struct v4l2_capability *cap)
{
	...
	strncpy(cap->driver, MTK_CAMERA_DRV_NAME, sizeof(cap->driver) - 1);
	strncpy(cap->bus_info, dev->platform, sizeof(cap->bus_info) - 1);
	strncpy(cap->card, MTK_CAMERA_DEV_NAME, sizeof(cap->card) - 1);
					...
					struct v4l2_format *format)
{
	...
	if (!fmt) {
		dev_err(ctx->dev, "find format failed\n");
		return -EINVAL;
	}
				    ...
				    struct mtk_camera_stream *stream)
{
	...
	ret = snprintf(video->name, sizeof(video->name), "%s@%d-%s",
		MTK_CAMERA_DEV_NAME, stream->camera_id, stream_type[stream->stream_id]);
	if (ret < 0) {
		dev_err(stream->ctx->dev, "Failed to compose video name err=%d\n", ret);
		return ret;
	}
...
static int mtk_camera_init_v4l2_device(struct mtk_camera_dev *camdev)
{
	...
	dev = camdev->dev;
	v4l2_dev = &camdev->v4l2_dev;
	v4l2_dev->mdev = &camdev->media_dev;

	strscpy(v4l2_dev->name, MTK_CAMERA_DEV_NAME,
...
static void mtk_camera_deinit_v4l2_device(struct mtk_camera_dev *camdev)
{
	if (!camdev) {
		pr_warn("%s: camdev is NULL\n", __func__);
		return;
	}
...
static int mtk_camera_init_media_device(struct mtk_camera_dev *camdev)
{
	...
	dev = camdev->dev;
	media_dev = &camdev->media_dev;
	media_dev->dev = dev;

	strscpy(media_dev->model, dev_driver_string(dev),
		sizeof(media_dev->model));

	strscpy(media_dev->bus_info, camdev->platform,
	...
	if (ret) {
		dev_err(dev, "failed to register media device ret=%d\n", ret);
		return ret;
	}
...
static void mtk_camera_deinit_media_device(struct mtk_camera_dev *camdev)
{
	if (!camdev) {
		pr_warn("%s: camdev is NULL\n", __func__);
		return;
	...
	media_device_unregister(&camdev->media_dev);
	media_device_cleanup(&camdev->media_dev);
}

static int mtk_camera_parse_dt(struct mtk_camera_dev *camdev)
{
	struct device *dev;
	int camera_id;
	unsigned int val;
...
static int mtk_camera_probe(struct platform_device *pdev)
{
	...
	ret = mtk_camera_parse_dt(camdev);
	if (ret) {
		dev_err(dev, "failed to parse dt ret=%d\n", ret);
		return ret;
	}

	ret = mtk_camera_init_v4l2_device(camdev);
	if (ret) {
		dev_err(dev, "failed to initialize v4l2 device ret=%d\n", ret);
		return ret;
	}

	ret = mtk_camera_init_media_device(camdev);
	if (ret) {
		dev_err(dev, "failed to initialize media device ret=%d\n", ret);
		goto err_init_media_device;
	}
...
err_create_stream:
	mtk_camera_deinit_media_device(camdev);

err_init_media_device:
	mtk_camera_deinit_v4l2_device(camdev);
