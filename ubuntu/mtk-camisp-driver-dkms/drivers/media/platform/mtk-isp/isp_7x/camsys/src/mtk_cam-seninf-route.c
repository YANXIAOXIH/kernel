...
int mtk_cam_seninf_get_vcinfo(struct seninf_ctx *ctx)
{
	...
	struct v4l2_subdev *sensor_sd = ctx->sensor_sd;
	struct v4l2_ctrl *ctrl;
	bool vc2pad = false; // for upstream sensor driver, using vc to decide the output pad
	...
	if (!ctrl) {
		...
		dev_dbg(ctx->dev, "%s, get_fmt sensor(pad=%d) dt=0x%x, hsize=%u, vsize=%u\n",
			__func__,
			sub_fmt.pad,
	...
	for (i = 0; i < fd.num_entries; i++) {
		...
		default:
			if (vc->dt == 0x2a || vc->dt == 0x2b ||
				...
				vc->feature = VC_RAW_DATA;
				vc->out_pad = (vc2pad) ? PAD_SRC_RAW0 + vc->vc : PAD_SRC_RAW0;
			} else {
...
static int _mtk_cam_seninf_set_camtg(struct v4l2_subdev *sd, int pad_id, int camtg, bool disable_last)
{
	...
	if (ctx->streaming && vc_en && vc->cam != camtg) {
		...
		if (ctx->is_secure == 1) {
			...
			} else if (camtg == 0x9) { // fix me: camsv8 issue
				old_camtg = vc->cam;
				vc->cam = camtg;
...
static void mtk_notify_vsync_fn(struct kthread_work *work)
{
	...
	if (!ctrl) {
		dev_dbg(ctx->dev, "%s, no V4L2_CID_VSYNC_NOTIFY %s\n",
			__func__,
			sensor_sd->name);
	} else {
