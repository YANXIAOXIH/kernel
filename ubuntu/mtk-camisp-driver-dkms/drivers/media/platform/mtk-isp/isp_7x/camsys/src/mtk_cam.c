					...
					int pipe_id, int index)
{
	...
	struct mtk_cam_video_device *node;
	struct vb2_buffer *vb;
	int i = 0, buf_state = 0, buf_start = 0, buf_end = 0, buf_ret_cnt = 0;
...
STOP_SCAN:
	...
	for (handled_cnt = 0; handled_cnt < s_data_cnt; handled_cnt++) {
		char tmp_debug_str[TASK_COMM_LEN + 11] = {0};
		...
		strscpy(tmp_debug_str, req->req.debug_str, sizeof(tmp_debug_str));
		...
		if (is_raw_subdev(pipe_id) && debug_ae) {
			...
			dev_info(ctx->cam->dev,
				...
				__func__, tmp_debug_str, ctx->stream_id, pipe_id,
		...
		if (s_data->frame_seq_no < dequeued_frame_seq_no) {
			...
			dev_dbg(ctx->cam->dev,
				...
				__func__, tmp_debug_str, pipe_id,
	...
	list_for_each_entry_safe(req, req_prev, &req_clean_list, cleanup_list) {
		...
		if (!s_data_pipe) {
			dev_dbg(cam->dev,
				...
				__func__, tmp_debug_str, req->pipe_used,
				pipe_id);
			continue;
		}
		if (mtk_cam_s_data_set_buf_state(s_data_pipe, buf_state)) {
				 ...
				 struct mtk_cam_request *cam_req)
{
	...
	for (rawi_idx = 0; rawi_idx < rawi_port_num; rawi_idx++) {
		...
		if (in_fmt->buf[0].iova == 0x0) {
			...
			if (!cfg_fmt) {
				dev_err(cam->dev, "%s: cfg_fmt is null", __func__);
				return;
			}
			       ...
			       struct mtk_cam_request_stream_data *s_data)
{
	...
	for (i = MTK_RAW_SINK_NUM + 1; i < MTK_RAW_META_OUT_BEGIN; i++) {
		...
		if (vfmt == NULL) {
			dev_info(pipe->subdev.v4l2_dev->dev, "error: %s: vfmt = NULL\n",
			__func__);
			return -EINVAL;
		}
...
static int mtk_cam_req_save_raw_pfmt(struct mtk_raw_pipeline *pipe,
			      struct mtk_cam_request *cam_req,
			      struct mtk_cam_request_stream_data *s_data)
{
	int pad;
...
static int mtk_cam_req_save_raw_psel(struct mtk_raw_pipeline *pipe,
			      struct mtk_cam_request *cam_req,
			      struct mtk_cam_request_stream_data *s_data)
{
	int pad;
...
static int mtk_cam_req_save_camsv_pfmt(struct mtk_camsv_pipeline *pipe,
			      struct mtk_cam_request *cam_req,
			      struct mtk_cam_request_stream_data *s_data)
{
	int pad;
	...
	s_data->pad_fmt_update = pipe->req_pfmt_update;
	/* save MEDIA_PAD_FL_SINK pad's fmt to request */
	for (pad = MTK_CAMSV_SINK_BEGIN; pad < MTK_CAMSV_SINK_NUM; pad++) {
		if (s_data->pad_fmt_update & 1 << pad)
			s_data->pad_fmt[pad] = pipe->req_pad_fmt[pad];
	}

	/* save MEDIA_PAD_FL_SOURCE pad's fmt to request */
	for (pad = MTK_CAMSV_SOURCE_BEGIN; pad < MTK_CAMSV_PIPELINE_PADS_NUM; pad++) {
		if (s_data->pad_fmt_update & 1 << pad)
			s_data->pad_fmt[pad] = pipe->req_pad_fmt[pad];
	}
				   ...
				   struct mtk_cam_buffer *buf)
{
	...
	if (!raw_pipline) {
		dev_err(cam->dev, "%s: raw_pipeline not found\n", __func__);
		return;
	}
			      ...
			      struct mtk_cam_buffer *buf)
{
	...
	if (cfg_selection) {
		...
		dev_dbg(cam->dev, "%s: cfg_selection is null", __func__);
	}
					...
					struct mtk_cam_request *req)
{
	...
	for (i = MTKCAM_SUBDEV_CAMSV_START; i < cam->max_stream_num; i++) {
		...
		if (!ctx->streaming) {
			dev_dbg(cam->dev,
				"%s: pipe_used(0x%x), stream_id(0x%x), streaming(%d)\n",
				__func__, req->pipe_used, ctx->stream_id,
				ctx->streaming);
			return 0;
		}
	}
	...
	list_for_each_entry_safe(req, req_prev, &cam->pending_job_list, list) {
		if (likely(mtk_cam_dev_req_is_stream_on(cam, req))) {
			if (job_count + enqueue_req_cnt >=
			    (RAW_PIPELINE_NUM + CAMSV_PIPELINE_NUM) * MTK_CAM_MAX_RUNNING_JOBS) {
...
static unsigned int mtk_cam_req_get_pipe_used_without_request(
	struct mtk_cam_request *cam_req)
{
	/**
	 * V4L2 framework doesn't trigger q->ops->buf_queue(q, buf) when it is
	...
	unsigned int i, feature;
	struct mtk_raw_pipeline *raw_pipeline;
	struct mtk_cam_device *cam = vb2_get_drv_priv(cam_req->vbq);
	struct mtk_cam_video_device *node = mtk_cam_vbq_to_vdev(cam_req->vbq);
...
static void mtk_cam_req_queue(struct media_request *req)
{
	...
	struct mtk_cam_device *cam =
		container_of(req->mdev, struct mtk_cam_device, media_dev);
	struct mtk_raw_pipeline *raw_pipeline;
	struct mtk_camsv_pipeline *camsv_pipeline;
	struct mtk_cam_request_stream_data *s_data;
	...
	for (i = MTKCAM_SUBDEV_RAW_0; i < MTKCAM_SUBDEV_RAW_END; i++) {
		if (cam_req->pipe_used & 1 << i) {
			if (i - MTKCAM_SUBDEV_RAW_0 >= RAW_PIPELINE_NUM) {
				dev_dbg(cam->dev,
	...
	for (i = MTKCAM_SUBDEV_CAMSV_START; i < MTKCAM_SUBDEV_CAMSV_END; i++) {
		if (cam_req->pipe_used & 1 << i) {
			camsv_pipeline = &cam->sv.pipelines[i - MTKCAM_SUBDEV_CAMSV_START];
			s_data = mtk_cam_req_get_s_data_no_chk(cam_req, i, 0);
			mtk_cam_req_save_camsv_pfmt(camsv_pipeline, cam_req, s_data);
		}
	}
				       ...
				       struct v4l2_subdev *req_seninf_new)
{
	...
	ret = snprintf(warn_desc, 60, "prev:%s: sensor_n(%p)/seninf_o(%p)/seninf_n(%p)",
...
void mtk_cam_req_queue_without_request(struct mtk_cam_request *cam_req)
{
	struct mtk_cam_device *cam = vb2_get_drv_priv(cam_req->vbq);

	/* reset done status */
	cam_req->done_status = 0;
	cam_req->pipe_used = mtk_cam_req_get_pipe_used_without_request(cam_req);
	cam_req->flags = 0;
	cam_req->ctx_used = 0; /* will update after stream on */
	mtk_cam_fs_reset(&cam_req->fs);
...
static void isp_tx_frame_worker(struct work_struct *work)
{
	...
	if (!s_raw_pipe_data) {
		dev_err(cam->dev, "%s: s_raw_pipe_data is null\n", __func__);
		return;
	}
					  ...
					  int stream_id)
{
	...
	if (!s_raw_pipe_data) {
		dev_err(ctx->cam->dev, "%s: s_raw_pipe_data is null\n", __func__);
		return;
	}
			    ...
			    struct mtkcam_ipi_input_param *cfg_in_param)
{
	...
	s_raw_pipe_data = mtk_cam_s_data_get_raw_pipe_data(s_data);
	if (!s_raw_pipe_data) {
		dev_dbg(raw->cam_dev, "%s: s_raw_pipe_data is null\n", __func__);
		return -EINVAL;
	}
...
void mtk_cam_apply_pending_dev_config(struct mtk_cam_request_stream_data *s_data)
{
	...
	s_raw_pipe_data = mtk_cam_s_data_get_raw_pipe_data(s_data);
	if (!s_raw_pipe_data) {
		dev_info(ctx->cam->dev, "%s: s_raw_pipe_data is null\n", __func__);
		return;
	}
	...
	bool streaming, bool config_pipe)
{
	...
	s_raw_pipe_data = mtk_cam_s_data_get_raw_pipe_data(s_data);
	if (!s_raw_pipe_data) {
		dev_dbg(dev, "%s: s_raw_pipe_data is null\n", __func__);
		return -EINVAL;
	}
	feature = s_raw_pipe_data->res.raw_res.feature;
...
int mtk_cam_s_data_sv_dev_config(struct mtk_cam_request_stream_data *s_data)
{
	...
	s_raw_pipe_data = mtk_cam_s_data_get_raw_pipe_data(s_data);
	if (!s_raw_pipe_data) {
		dev_info(cam->dev, "%s: s_raw_pipe_data is null", __func__);
		return -EINVAL;
	}
	...
	for (i = MTKCAM_SUBDEV_CAMSV_START; i < MTKCAM_SUBDEV_CAMSV_END; i++) {
		...
		if (used_pipes & (1 << i)) {
			if (i - MTKCAM_SUBDEV_CAMSV_START >= MAX_SV_PIPES_PER_STREAM) {
				dev_info(cam->dev, "erro: %s: illegal paramete", __func__);
				return -EINVAL;
			}
...
static int isp_composer_init(struct mtk_cam_ctx *ctx, unsigned int pipe_id)
{
	...
	ret = snprintf(msg->name, RPMSG_NAME_SIZE, "mtk-camsys\%d", pipe_id);
				     ...
				     struct media_entity *entity)
{
	...
	// First, search for a context with the same or adjacent entity
	for (i = 0;  i < cam->max_stream_num; i++) {
		spin_lock(&cam->ctxs[i].entity_lock);
		if (cam->ctxs[i].connect_bridge && cam->ctxs[i].entity) {
			if (entity == cam->ctxs[i].entity) {
				spin_unlock(&cam->ctxs[i].entity_lock);
				return &cam->ctxs[i];
			}

			for (j = 0; j < cam->ctxs[i].entity->num_pads; j++) {
				struct media_pad *local_pad = &cam->ctxs[i].entity->pads[j];
				struct media_pad *remote_pad = media_entity_remote_pad(local_pad);

				if (remote_pad && entity == remote_pad->entity) {
					spin_unlock(&cam->ctxs[i].entity_lock);
					return &cam->ctxs[i];
				}
			}
	...
	// Next, look for a context with an identical pipeline
	for (i = 0;  i < cam->max_stream_num; i++) {
		spin_lock(&cam->ctxs[i].entity_lock);
		if (cam->ctxs[i].entity && (entity->pads->pipe == cam->ctxs[i].entity->pads->pipe)) {
			spin_unlock(&cam->ctxs[i].entity_lock);
			return &cam->ctxs[i];
		}
		spin_unlock(&cam->ctxs[i].entity_lock);
				      ...
				      struct mtk_cam_video_device *node)
{
	...
	for (i = 0; i < entity->num_pads; i++) {
		local_pad = &entity->pads[i];
		remote_pad = media_entity_remote_pad(local_pad);

		if (remote_pad) {
			remote_entity = remote_pad->entity;
			ctx->entity = remote_entity;
		}
	}

	mutex_lock(&cam->streaming_lock);
	list_for_each_entry(pipe_node, &cam->pipeline_list, list) {
		if (entity->pads->pipe == &pipe_node->pipeline) {
			mutex_unlock(&cam->streaming_lock);
			goto add_node_to_ctx;
		}
	}
	mutex_unlock(&cam->streaming_lock);

	pipe_node = kzalloc(sizeof(struct mtk_cam_media_pipeline), GFP_KERNEL);
	if (!pipe_node)
	...
	/* traverse to seninf and check what type of entity it is connected to */
	...
	while ((pad = media_graph_walk_next(graph))) {
		...
		if (entity->function == MEDIA_ENT_F_VID_IF_BRIDGE &&
		    strstr(entity->name, "seninf")) {
			for (i = 0; i < entity->num_pads; i++) {
				remote_pad = media_entity_remote_pad(&entity->pads[i]);

				if (remote_pad &&
				    remote_pad->entity->function == MEDIA_ENT_F_VID_IF_BRIDGE) {
					dev_info(cam->dev,
						"%s: seninf is connected to MEDIA_ENT_F_VID_IF_BRIDGE subdev\n",
						__func__);
					pipe_node->connect_bridge = connect_bridge = true;
				}
			}
			break;
		}
	...
	while ((pad = media_graph_walk_next(graph))) {
		entity = pad->entity;
		dev_dbg(cam->dev, "linked entity %s func=0x%x\n",
		...
		case MEDIA_ENT_F_IO_V4L:
			pipe_node->enabled_node_cnt++;
			break;
		case MEDIA_ENT_F_PROC_VIDEO_PIXEL_FORMATTER: /* pipeline */
			if (i >= MAX_PIPES_PER_STREAM)
				goto fail_stop_pipeline;

			target_sd = pipe_node->pipe_subdevs + i;
			i++;
			break;
		case MEDIA_ENT_F_VID_IF_BRIDGE: /* seninf */
			if (strstr(entity->name, "seninf"))
				target_sd = &pipe_node->seninf;
			break;
		case MEDIA_ENT_F_CAM_SENSOR:
			if (!connect_bridge)
				target_sd = &pipe_node->sensor;
			break;
		default:
			break;
		}
	...
	mutex_lock(&cam->streaming_lock);
	list_add_tail(&pipe_node->list, &cam->pipeline_list);
	mutex_unlock(&cam->streaming_lock);

add_node_to_ctx:
	ctx->connect_bridge = pipe_node->connect_bridge;
	ctx->enabled_node_cnt = pipe_node->enabled_node_cnt;
	ctx->sensor = pipe_node->sensor;
	ctx->seninf = pipe_node->seninf;
	for (i = 0; i < MAX_PIPES_PER_STREAM; i++)
		ctx->pipe_subdevs[i] = pipe_node->pipe_subdevs[i];
	pipe_node->counter++;

	if (ctx->connect_bridge)
...
{
	struct mtk_cam_device *cam = ctx->cam;
	struct mtk_cam_media_pipeline *curr_node;
	struct mtk_cam_media_pipeline *temp_node;
	unsigned int i, j;
	...
	mutex_lock(&cam->streaming_lock);
	list_for_each_entry_safe(curr_node, temp_node, &cam->pipeline_list, list) {
		if (entity->pads->pipe == &curr_node->pipeline) {
			curr_node->counter--;
			if (!curr_node->counter) {
	...
	/* When there are more than two pipelines in use, we should clear the state
	 * only after an entire pipeline has been shut down. However, since v4l2_dev
	 * records all devices collectively, it is challenging to distinguish between
	 * these devices and determine when a single pipeline has been completely
	 * turned off. The current approach is to avoid performing stream off
	 * operations on devices simultaneously to prevent state inconsistencies.
	 */
	mutex_lock(&cam->streaming_lock);
	if (!cam->streaming_ctx) {
		struct v4l2_subdev *sd;
	...
	ctx->connect_bridge = 0;
	ctx->enabled_node_cnt = 0;
	ctx->streaming_node_cnt = 0;
	ctx->streaming_pipe = 0;
	spin_lock(&ctx->entity_lock);
	ctx->entity = NULL;
	spin_unlock(&ctx->entity_lock);
	ctx->sensor = NULL;
...
int mtk_cam_ctx_stream_on(struct mtk_cam_ctx *ctx)
{
	...
	if (!ctx->seninf) {
		dev_err(cam->dev, "ctx %d has no valid seninf(%p)\n",
			ctx->stream_id, ctx->seninf);
		return -EINVAL;
	}

	for (i = 0; i < MAX_PIPES_PER_STREAM && ctx->pipe_subdevs[i]; i++) {
		if (ctx->connect_bridge &&
		    ctx->entity != &ctx->pipe_subdevs[i]->entity)
			continue;
	...
	if (ctx->pipe) {
		...
		dev_info(cam->dev, "%s: ctx-%d pipe not found, used_raw_num = %d\n", __func__, ctx->stream_id, ctx->used_raw_num);
	}
	...
	if (!mtk_cam_feature_is_m2m(feature_active)) {
		for (i = 0 ; i < ctx->used_sv_num ; i++) {
			...
			/* Temporary workaround for an issue with SerDes MAX9286.
			 * There is a probability that after N switch cycles,
			 * the first stream on may result in a blank image.
			 */
			if (!ctx->connect_bridge)
	...
	dev_info(cam->dev, "streamed on camsys ctx:%d\n", ctx->stream_id);
...
int mtk_cam_ctx_stream_off(struct mtk_cam_ctx *ctx)
{
	...
	if (!ctx->streaming) {
		dev_info(cam->dev, "%s: ctx-%d is already streaming off\n",
			__func__, ctx->stream_id);
		return 0;
	}
	...
	for (i = 0; i < cam->max_stream_num; i++) {
		if (i >= MTKCAM_SUBDEV_RAW_START &&
			...
			if (i >= RAW_PIPELINE_NUM) {
				dev_info(cam->dev, "error: %s: invalid parameter\n", __func__);
				return -EINVAL;
			}
			...
			for (j = PAD_SRC_RAW0; j < PAD_SRC_RAW_EXT0; j++) {
...
static void mtk_cam_ctx_watchdog_worker(struct work_struct *work)
{
	...
	if (!raw) {
		dev_info(ctx->cam->dev,
			 "%s:ctx(%d):stop watchdog task for no raw\n",
			 __func__, ctx->stream_id);
		return;
	}
			     ...
			     unsigned int stream_id)
{
	...
	spin_lock_init(&ctx->entity_lock);
...
static int mtk_cam_probe(struct platform_device *pdev)
{
	...
	INIT_LIST_HEAD(&cam_dev->pipeline_list);

	mutex_init(&cam_dev->queue_lock);
	mutex_init(&cam_dev->streaming_lock);
...
fail_destroy_mutex:
	...
	mutex_destroy(&cam_dev->streaming_lock);
