				   ...
				   struct mtk_cam_request_stream_data *s_data)
{
	...
	if (mtk_cam_is_stagger(ctx)) {
		...
		if (mtk_cam_is_hsf(ctx))
			mtk_cam_seninf_set_secure(s_data->seninf_new, 1,
				      ...
				      struct mtk_camsys_sensor_ctrl *sensor_ctrl)
{
	...
	dev_dbg(cam->dev, "%s:%s:ctx(%d)req(%d):sensor done at SOF+%dms\n",
		__func__, req->req.debug_str, ctx->stream_id,
		data->frame_seq_no, time_after_sof);
	mtk_cam_tg_flash_req_setup(ctx, data);
	mtk_cam_complete_sensor_hdl(data);
}
			      ...
			      unsigned int pipe_id)
{
	...
	req_stream_data = mtk_cam_get_req_s_data(ctx, ctx->stream_id,
						dequeued_frame_seq_no);
	if (mtk_cam_is_mstream_m2m(ctx) && req_stream_data) {
				       ...
				       struct mtk_camsys_irq_info *irq_info)
{
	...
	if (ctx->sensor) {
		...
		if (state_handle_ret != STATE_RESULT_TRIGGER_CQ) {
			...
			if (ctx->pipe->stagger_path == STAGGER_DCIF) {
				...
				struct mtk_cam_img_working_buf_entry *img_buf_entry;
				struct mtk_camsv_device *camsv_dev;
				struct device *dev_sv;
				int sv_i;
				...
				for (sv_i = MTKCAM_SUBDEV_CAMSV_START;
					...
					if (ctx->pipe->enabled_raw & (1 << sv_i)) {
						...
						img_buf_entry = mtk_cam_img_working_buf_get(ctx);
						if (img_buf_entry) {
							mtk_cam_sv_enquehwbuf(camsv_dev,
								img_buf_entry->img_buffer.iova,
								dequeued_frame_seq_no);
							dev_info(camsv_dev->dev,
								"[%s] dcif stagger workaround camsv id:%d/iova:0x%llx\n",
								__func__, camsv_dev->id,
								img_buf_entry->img_buffer.iova);
							mtk_cam_img_working_buf_put(img_buf_entry);
						}
					}
				}
			}
	...
	if (list_empty(&ctx->composed_buffer_list.list)) {
		...
		buf_entry = list_first_entry(&ctx->composed_buffer_list.list,
					     ...
					     list_entry);
		if (!buf_entry) {
			dev_err(raw_dev->dev, "[%s] buf_entry not found, deq:%d\n", __func__, dequeued_frame_seq_no);
			return;
		}
		list_del(&buf_entry->list_entry);
		ctx->composed_buffer_list.cnt--;
		...
		/* update qos bw */
		if (virt_addr_valid(req_stream_data))
			mtk_cam_qos_bw_calc(ctx, req_stream_data->raw_dmas);
				   ...
				   unsigned int frame_seq_no_outer)
{
	...
	if (raw_dev->sof_count == 0) {
		...
		if (!req_stream_data) {
			dev_err(raw_dev->dev, "[%s] req_stream_data is null", __func__);
			return;
		}
							...
							unsigned int pipe_id)
{
	...
	if (!req_stream_data) {
		dev_err(ctx->cam->dev, "[%s] req_stream_data is null", __func__);
		return;
	}
				       ...
				       struct mtk_camsys_irq_info *irq_info)
{
	...
	if (!ctx) {
		dev_dbg(raw_dev->dev, "[%s] cannot find ctx\n", __func__);
		return -EINVAL;
	}
					 ...
					 struct mtk_camsys_irq_info *irq_info)
{
	...
	if (camsv_dev->pipeline->hw_scen &
		...
		if (!ctx) {
			dev_err(camsv_dev->dev, "[%s] cannot find ctx", __func__);
			return -EINVAL;
		}
				...
				if (ctx->pipe->stagger_path == STAGGER_DCIF) {
					...
					if (raw_dev->sof_count == 1) {
						...
						if (camsv_dev_s)
							...
							dev_dbg(camsv_dev->dev, "[%s] get camsv_dev_s fail\n", __func__);
					}
				}
#endif
				mtk_camsys_raw_frame_start(raw_dev, ctx, irq_info);
		...
		if (!ctx) {
			dev_dbg(camsv_dev->dev, "[%s] cannot find ctx\n", __func__);
			return -EINVAL;
		}
