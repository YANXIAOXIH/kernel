...
static int mtk_cam_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	switch (ctrl->id) {
...
static const struct v4l2_ctrl_ops mtk_cam_ctrl_ops = {
	.g_volatile_ctrl = mtk_cam_g_volatile_ctrl,
};
				   ...
				   struct device *alloc_devs[])
{
	...
	dev_dbg(cam->dev, "%s: width=%u height=%u fourcc=0x%x sizeimage=%d\n",
		__func__, fmt->fmt.pix_mp.width, fmt->fmt.pix_mp.height,
		fmt->fmt.pix_mp.pixelformat, size);
...
static int mtk_cam_vb2_buf_prepare(struct vb2_buffer *vb)
{
	...
	if (!vb->request) {
		...
		dev_dbg(vb->vb2_queue->dev, "%s: %s no request_fd, create dummy request\n",
			__func__, node->desc.name);
		...
		spin_lock_init(&req->done_status_lock);
		mutex_init(&req->fs.op_lock);
		req->vbq = vb->vb2_queue;

		req->req.state = MEDIA_REQUEST_STATE_IDLE;
		req->req.mdev = &cam->media_dev;
		kref_init(&req->req.kref);
		INIT_LIST_HEAD(&req->req.objects);
		spin_lock_init(&req->req.lock);
		init_waitqueue_head(&req->req.poll_wait);
		ret = snprintf(req->req.debug_str, sizeof(req->req.debug_str),
			       "dummy-%u:%d", node->uid.pipe_id, vb->index);
		if (ret >= sizeof(req->req.debug_str))
			dev_dbg(vb->vb2_queue->dev, "%s: string truncated\n", __func__);
		buf->req = req;
		mtk_cam_req_queue_without_request(req);
	}
				       ...
				       unsigned int count)
{
	...
	if (!ctx->connect_bridge && (ctx->streaming_node_cnt < ctx->enabled_node_cnt))
		return 0;
...
fail_return_buffer:
	...
	mtk_cam_dev_req_cleanup(node->ctx, node->uid.pipe_id, VB2_BUF_STATE_QUEUED);
	return ret;
}
...
static int mtk_video_init_ctrls(struct mtk_cam_video_device *video)
{
	int ret = 0;
	struct v4l2_ctrl *ctrl = NULL;

	ret = v4l2_ctrl_handler_init(&video->ctrl_hdlr, 5);
	if (ret)
		return ret;

	if (is_camsv_subdev(video->uid.pipe_id)) {
		ctrl = v4l2_ctrl_new_std(&video->ctrl_hdlr,
					&mtk_cam_ctrl_ops,
		...
		if (ctrl) {
			...
			ret = video->ctrl_hdlr.error;
			return ret;
		}
	}

	ret = v4l2_ctrl_handler_setup(&video->ctrl_hdlr);
	if (ret)
		return ret;

	video->vdev.ctrl_handler = &video->ctrl_hdlr;
			   ...
			   struct v4l2_device *v4l2_dev)
{
	...
	if (video->desc.smem_alloc) {
		...
		/*TODO: workaround for kernel panic
	...
	ret = mtk_video_init_ctrls(video);
	if (ret < 0) {
		dev_info(v4l2_dev->dev, "Failed to init controls: %d\n", ret);
		goto error_video_register;
	}
	...
	vdev->device_caps = video->desc.cap | V4L2_CAP_STREAMING | V4L2_CAP_IO_MC;
...
void mtk_cam_video_unregister(struct mtk_cam_video_device *video)
{
	...
	v4l2_ctrl_handler_free(&video->ctrl_hdlr);
	vb2_queue_release(&video->vb2_q);
			    ...
			    struct v4l2_capability *cap)
{
	...
	ret = snprintf(cap->bus_info, sizeof(cap->bus_info), "platform:%s",
		       dev_name(cam->dev));
	if (ret >= sizeof(cap->bus_info))
		dev_dbg(cam->dev, "%s: string truncated\n", __func__);
			    ...
			    struct v4l2_fmtdesc *f)
{
	...
	unsigned int i = f->index;

	/*
	 * If mbus_code is set, only enumerate supported pixel formats
	 * corresponding to that bus code.
	 */
	if (f->mbus_code) {
		for (i = 0; i < node->desc.num_fmts; ++i) {
			if (node->desc.fmts[i].pfmt.code != f->mbus_code)
				continue;
	...
	f->pixelformat = node->desc.fmts[i].vfmt.fmt.pix_mp.pixelformat;
	f->flags = 0;
...
static int mtk_cam_collect_vfmt(struct mtk_raw_pipeline *pipe,
			   ...
			   struct v4l2_format *f)
{
	...
	if (is_raw_subdev(node->uid.pipe_id) && raw_pipeline)
		raw_feature = raw_pipeline->user_res.raw_res.feature;
