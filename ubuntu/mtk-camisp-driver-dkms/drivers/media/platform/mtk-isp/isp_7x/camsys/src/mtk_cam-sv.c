			  ...
			  struct v4l2_subdev_format *fmt)
{
	struct mtk_camsv_pipeline *pipe =
		container_of(sd, struct mtk_camsv_pipeline, subdev);
	struct mtk_camsv *sv = pipe->sv;
	unsigned int sensor_fmt = mtk_cam_get_sensor_fmt(to_std_fmt_code(fmt->format.code));
...
void sv_reset(struct mtk_camsv_device *dev)
{
	...
	/* To use the second CAMSV under the same GCAMSV_TOP independently */
	/* you need to set the dcm_dis of the first CAMSV to 0x7*/
	if (dev->base_top) {
		regmap_write(dev->base_top, REG_CAMSV_DCM_DIS, 0x7);
		dev_dbg(dev->dev, "%s camsv_id:%d disable first camsv dcm\n", __func__, dev->id);
	}

	/* Disable all DMA DCM before reset */
	writel(0x7, dev->base + REG_CAMSV_DCM_DIS);
	...
	if (ret < 0) {
		dev_info(dev->dev,
			"camsv_id: %d %s: timeout dcm_status: 0x%x\n",
			dev->id, __func__,
			readl(dev->base + REG_CAMSV_DCM_STATUS));
	...
	unsigned int exp_order)
{
	...
	struct v4l2_format *img_fmt = NULL;
	unsigned int i;
	...
	if (!img_fmt || !mf) {
		dev_err(dev, "config camsv img_fmt not found\n");
		return -EINVAL;
	}
			    ...
			    struct mtk_camsv_device *sv)
{
	...
	/* Obtain top regmap */
	top_node = of_parse_phandle(dev->of_node, "mediatek,camsv-top", 0);
	if (top_node) {
		sv->base_top = device_node_to_regmap(top_node);
		of_node_put(top_node);
		if (IS_ERR(sv->base_top)) {
			dev_dbg(dev, "failed to get register top base\n");
			return PTR_ERR(sv->base_top);
		}
	}
