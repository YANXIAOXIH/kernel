			      ...
			      size_t count, loff_t *ppos)
{
	...
	if (!param_str_0) {
		dev_err(ctrl->debug_fs->cam->dev, "%s: fail to get param_str_0\n", __func__);
		ret = -EFAULT;
		goto FAIL;
	}
	...
	if (cmd_str[0] == 'r') {
		...
		if (!param_str_1) {
			dev_err(ctrl->debug_fs->cam->dev, "%s: fail to get param_str_1\n", __func__);
			ret = -EFAULT;
			goto FAIL;
		}
			...
			size_t count, loff_t *ppos)
{
	...
	if (!cam) {
		dev_err(dev, "%s: cam can't be null\n", __func__);
		return 0;
	}

	debug_fs = cam->debug_fs;
	if (!debug_fs) {
		dev_err(dev, "%s: debug_fs can't be null\n", __func__);
		return 0;
	}

	if (!debug_fs->exp_dump_buf) {
		dev_err(dev, "%s: dump buf can't be null\n", __func__);
		return 0;
	}
...
static void mtk_cam_exception_work(struct work_struct *work)
{
	...
	if (ret >= 48)
		dev_dbg(ctx->cam->dev, "%s: string truncated\n", __func__);
		     ...
		     unsigned int dump_flag, char *desc, bool smi_dump)
{
	...
	strscpy(dbg_work->desc, desc, sizeof(dbg_work->desc));
...
mtk_cam_debug_seninf_dump(struct mtk_cam_request_stream_data *s_data)
{
	...
	if (!dump_work) {
		err = -ENOMEM;
		dev_err(ctx->cam->dev, "failed to alloc dump_work\n");
		return;
	}
