			  ...
			  struct seninf_vcinfo *vcinfo)
{
	...
	if (intf < 0) {
		dev_info(ctx->dev, "%s, invalid intf.\n", __func__);
		return -EINVAL;
	}
	pSeninf_csi2 = ctx->reg_if_csi2[intf];
...
static int mtk_cam_seninf_disable_all_mux(struct seninf_ctx *ctx)
{
	...
	for (i = start_seninf_mux; i < _seninf_ops->mux_num; i++) {
				  ...
				  int mux, int cam_mux, int pixel_mode)
{
	...
	intf = mux % _seninf_ops->seninf_num; /* XXX: only a subset of seninf has a testmdl, by platform */
...
static int csirx_dphy_init(struct seninf_ctx *ctx)
{
	...
	dev_info(ctx->dev, "SENINF settle(0x%x),trail:0x%x\n",
		ctx->dphy_settle_delay_dt,
	...
	if (!ctx->is_cphy) {
		...
		hs_trail_en = data_rate < 850000000;
...
static int mtk_cam_seninf_reset(struct seninf_ctx *ctx, int seninfIdx)
{
	...
	if (seninfIdx < 0 || seninfIdx >= _seninf_ops->seninf_num) {
		dev_info(ctx->dev, "invalid seninfIdx = %d", seninfIdx);
		return -EINVAL;
	}

	pSeninf = ctx->reg_if_ctrl[seninfIdx];
	...
	if (core->csi_irq_en_flag) {
		list_for_each_entry(ctx, &core->list, list) {
			...
			if ((ctx->data_not_enough_cnt) >= (core->detection_cnt) ||
				...
				if (cnt1 >= 256) {
					dev_dbg(ctx->dev, "%s: Buffer full, stopping writes\n", __func__);
					goto END_SNPRINTF;
				}
				cnt_tmp += cnt1;
