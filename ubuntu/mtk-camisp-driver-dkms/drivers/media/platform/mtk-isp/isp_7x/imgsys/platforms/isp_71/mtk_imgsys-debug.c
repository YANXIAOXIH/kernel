...
void imgsys_main_init(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	ret = of_address_to_resource(imgsys_dev->dev->of_node, REG_MAP_E_ADL_A, &adl);
	if (ret) {
		dev_info(imgsys_dev->dev, "%s Unable to get resource from devnode(%s)\n",
			 __func__, imgsys_dev->dev->of_node->name);
		return;
	}
...
void imgsys_main_set_init(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	/* 1. dip_top_sw_rst = 0xffffffff */
	iowrite32(0xFFFFFFFF, (void *)(dipRegBA + SW_RST));
	/* 2. dip_nr_sw_rst = 0xffffffff */
	iowrite32(0xFFFFFFFF, (void *)(dip1RegBA + SW_RST));

	/* 3. wpe0_sw_rst on/off */
	iowrite32(0xFFFFFFFF, (void *)(wpedip1RegBA + SW_RST));
	iowrite32(0x0, (void *)(wpedip1RegBA + SW_RST));
	/*4. wpe1_sw_rst on/off */
	iowrite32(0xFFFFFFFF, (void *)(wpedip2RegBA + SW_RST));
	iowrite32(0x0, (void *)(wpedip2RegBA + SW_RST));
	/*5. wpe3_sw_rst on/off */
	iowrite32(0xFFFFFFFF, (void *)(wpedip3RegBA + SW_RST));
	iowrite32(0x0, (void *)(wpedip3RegBA + SW_RST));
...
void imgsys_dl_debug_dump(struct mtk_imgsys_dev *imgsys_dev, unsigned int hw_comb)
{
	...
	for (i = 0 ; i < DL_CHECK_ENG_NUM ; i++) {
		...
		if (hw_comb & dbg_engine_name_list[i].eng_e) {
			if (get) {
				ret = snprintf(logBuf_eng, log_length, "-%s",
	...
	memset((char *)logBuf_eng, 0x0, log_length);
	logBuf_eng[strlen(logBuf_eng)] = '\0';
	strscpy(logBuf_eng, " FAIL", log_length);
	strncat(logBuf_path, logBuf_eng, strlen(logBuf_eng));
	...
	case (IMGSYS_ENG_WPE_EIS | IMGSYS_ENG_TRAW):
		...
		strscpy(logBuf_inport, "WPE_EIS", log_length);
		strscpy(logBuf_outport, "TRAW", log_length);
	...
	case (IMGSYS_ENG_WPE_EIS | IMGSYS_ENG_LTR):
		dl_path = IMGSYS_DL_WPEE_TRAW;
		strscpy(logBuf_inport, "WPE_EIS", log_length);
		strscpy(logBuf_outport, "LTRAW", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		break;
	case (IMGSYS_ENG_WPE_EIS | IMGSYS_ENG_XTR):
		...
		strscpy(logBuf_inport, "WPE_EIS", log_length);
		strscpy(logBuf_outport, "XTRAW", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		break;
	case (IMGSYS_ENG_WPE_TNR | IMGSYS_ENG_TRAW):
		dl_path = IMGSYS_DL_WPET_TRAW;
		strscpy(logBuf_inport, "WPE_TNR", log_length);
		strscpy(logBuf_outport, "TRAW", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		break;
	...
	case (IMGSYS_ENG_WPE_LITE | IMGSYS_ENG_TRAW):
		dl_path = IMGSYS_DL_WPET_TRAW;
		strscpy(logBuf_inport, "WPE_LITE", log_length);
		strscpy(logBuf_outport, "TRAW", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		dev_info(imgsys_dev->dev,
	...
	case (IMGSYS_ENG_WPE_EIS | IMGSYS_ENG_DIP | IMGSYS_ENG_PQDIP_A |
		...
		strscpy(logBuf_inport, "WPE_EIS", log_length);
		strscpy(logBuf_outport, "DIP", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		/**/
		memset((char *)logBuf_inport, 0x0, log_length);
		...
		strscpy(logBuf_inport, "DIP", log_length);
		strscpy(logBuf_outport, "PQDIPA", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		/**/
		memset((char *)logBuf_inport, 0x0, log_length);
		...
		strscpy(logBuf_inport, "DIP", log_length);
		strscpy(logBuf_outport, "PQDIPB", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		break;
	...
	case (IMGSYS_ENG_WPE_EIS | IMGSYS_ENG_PQDIP_A |
		...
		strscpy(logBuf_inport, "WPE_EIS", log_length);
		strscpy(logBuf_outport, "PQDIP", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		break;
	...
	case (IMGSYS_ENG_WPE_EIS | IMGSYS_ENG_TRAW | IMGSYS_ENG_DIP |
		...
		strscpy(logBuf_inport, "TRAW", log_length);
		strscpy(logBuf_outport, "DIP", log_length);
		imgsys_dl_checksum_dump(imgsys_dev, hw_comb,
			logBuf_path, logBuf_inport, logBuf_outport, dl_path);
		/**/
