							...
							unsigned int count)
{
	...
	if (!pipe->nodes_streaming) {
		ret = media_pipeline_start(node->vdev.entity.pads, &pipe->pipeline);
		if (ret < 0) {
			dev_info(pipe->imgsys_dev->dev,
				   ...
				   struct v4l2_capability *cap)
{
	...
	if (ret >= sizeof(cap->bus_info))
		dev_dbg(pipe->imgsys_dev->dev, "%s: string truncated\n", __func__);
...
static bool get_user_by_file(struct file *filp, struct mtk_imgsys_user **user)
{
	...
	if (user == NULL) {
		pr_err("%s: user is NULL!", __func__);
		return found;
	}
...
static int mtkdip_ioc_add_kva(struct v4l2_subdev *subdev, void *arg)
{
	...
	for (i = 0; i < fd_info->fd_num; i++) {
		...
		if (ret) {
			pr_err("%s:dma_buf_vmap fail ret:%d\n", __func__, ret);
...
static void mtk_imgsys_request_free(struct media_request *req)
{
	...
	if (imgsys_req->used) {
		if (wait_for_completion_timeout(&imgsys_req->done, HZ) == 0)
			pr_info("%s: complete timeout\n", __func__);
	}
					   ...
					   struct mtk_imgsys_video_device *node)
{
	...
	ret = snprintf(vdev->name, sizeof(vdev->name), "%s %s", pipe->desc->name,
		 node->desc->name);
	if (ret >= sizeof(vdev->name))
		dev_dbg(pipe->imgsys_dev->dev, "%s: string truncated\n", __func__);
			       ...
			       struct v4l2_device *v4l2_dev)
{
	...
	strscpy(pipe->subdev.name, pipe->desc->name, sizeof(pipe->subdev.name));
...
static int mtk_imgsys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < larbs_num; i++) {
		...
		if (!link)
			...
			dev_info(imgsys_dev->dev, "ok : to link SMI LARB idx %d\n", i);
