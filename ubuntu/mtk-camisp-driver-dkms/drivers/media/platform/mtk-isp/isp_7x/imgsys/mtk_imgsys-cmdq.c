...
#define IMGSYS_CMDQ_CMD_BUF_SIZE (0x4000)
...
void imgsys_cmdq_init(struct mtk_imgsys_dev *imgsys_dev, const int nr_imgsys_dev)
{
	...
	case 1: /* DIP */
		...
		for (idx = 0; idx < IMGSYS_ENG_MAX; idx++) {
			...
			pr_info("%s: cmdq_mbox_create(%d, %p)\n", __func__, idx, imgsys_clt[idx]);
		}
		#if IMGSYS_SECURE_ENABLE
...
void imgsys_cmdq_streamon(struct mtk_imgsys_dev *imgsys_dev)
{
	u32 idx = 0;
#if !CMDQ_EXT
	struct cleartoken_info_t cleartoken_info = { 0 };
#endif
	...
	for (idx = IMGSYS_CMDQ_SYNC_TOKEN_IMGSYS_START;
		idx <= IMGSYS_CMDQ_SYNC_TOKEN_IMGSYS_END; idx++) {
		cleartoken_info.token[idx - IMGSYS_CMDQ_SYNC_TOKEN_IMGSYS_START] = idx;
		cleartoken_info.clearnum++;
	}

	imgsys_cmdq_clearevent(&cleartoken_info);
#endif
...
#if !CMDQ_EXT
static void imgsys_cmdq_cleartoken_cb(struct cmdq_cb_data data)
{
	...
	cb_param->err = data.sta; //data.err;

	if (cb_param->err != 0)
		pr_err("%s: [ERROR] cb(%p) for imgsys_cmdq_clearevent !\n",
			__func__, cb_param);
	else
		pr_debug("%s: [SUCCESS] cb(%p) for imgsys_cmdq_clearevent !\n",
			__func__, cb_param);
...
static void imgsys_cmdq_task_cb(struct cmdq_cb_data data)
{
	struct mtk_imgsys_cb_param *cb_param;
	...
	for (frm_idx = 0; frm_idx < frm_num; frm_idx++) {
		...
		for (blk_idx = 0; blk_idx < cmd_buf->frame_block; blk_idx++) {
			...
			if (isPack == 0) {
				...
				pkt = cmdq_pkt_create(clt, IMGSYS_CMDQ_CMD_BUF_SIZE);
				if (pkt == NULL) {
					pr_info(
				...
				pr_debug(
					"%s: cmdq_pkt_create success(%p) in block(%d) for frm(%d/%d)\n",
...
#else
void imgsys_cmdq_clearevent(struct cleartoken_info_t *cleartoken_info)
{
	struct cmdq_client *clt = NULL;
	struct cmdq_pkt *pkt = NULL;
	struct mtk_imgsys_cb_param *cb_param = NULL;
	...
	/* use imgsys_clt[0] mbox for imgsys_cmdq_clearevent */
	clt = imgsys_clt[0];
	...
	if (cb_param == NULL) {
		cmdq_pkt_destroy(pkt);
		pr_err("%s: [ERROR] cb_param vzalloc fail!\n", __func__);
		return;
	}
	...
	for (i = 0 ; i < cleartoken_info->clearnum ; i++) {
		pr_debug("%s:force clear swevent(%d).\n",
			__func__, cleartoken_info->token[i]);
		cmdq_pkt_clear_event(pkt, imgsys_event[cleartoken_info->token[i]].event);
	}

	cmdq_pkt_finalize(pkt);
	ret_flush = cmdq_pkt_flush_async(pkt, imgsys_cmdq_cleartoken_cb, (void *)cb_param);

	if (ret_flush < 0)
		pr_err("%s: [ERROR] cmdq_pkt_flush_async fail(%d) !\n", __func__, ret_flush);
	else
		pr_debug("%s: [SUCCESS] cmdq_pkt_flush_async success(%d) !\n", __func__, ret_flush);
}
#endif
