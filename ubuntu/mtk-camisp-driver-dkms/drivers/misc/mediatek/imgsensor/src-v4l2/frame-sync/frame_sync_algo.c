	...
	int idx, int m_idx, struct FrameSyncDynamicPara *p_para)
{
	...
	p_para->f_tag = fs_inst[(unsigned int)idx].frame_tag;
	p_para->f_cell = fs_inst[(unsigned int)idx].frame_cell_size;
}
...
void fs_alg_sa_notify_setup_all_frame_info(unsigned int idx)
{
	...
	if (idx != m_idx)
		frm_timestamp_checker(fs_inst[(unsigned int)m_idx].tg, fs_inst[idx].tg);
}
