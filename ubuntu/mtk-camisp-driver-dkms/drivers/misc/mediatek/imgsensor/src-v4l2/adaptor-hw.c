...
static struct mtk_gpio_desc *g_gpio_desc;
...
static struct clk *get_clk_by_freq(struct adaptor_ctx *ctx, int freq)
{
	...
	default:
		dev_warn(ctx->dev, "clk freq %d not supported\n", freq);
		return NULL;
	}
}
...
static int set_mclk(struct adaptor_ctx *ctx, void *data, int val)
{
	...
	if (mclk_src == NULL) {
		dev_err(ctx->dev, "failed to get mclk src\n");
		return -EINVAL;
	}
...
static int set_gpio(struct adaptor_ctx *ctx, void *data, int val)
{
	...
	idx = (unsigned long long)data;
	gpio_desc = ctx->gpio[idx];
	dev_dbg(ctx->dev, "set gpio %s,val %d\n", gpio_names[idx], val);

	/* gpio states */
	ret = gpiod_direction_output(gpio_desc, val);
	if (ret < 0) {
		dev_dbg(ctx->dev, "fail to set gpio %s\n", gpio_names[idx]);
		return ret;
	}
	if (gpio_desc == g_gpio_desc->gpio[idx])
		g_gpio_desc->gpio_pwr_cnt[idx]++;

	return 0;
}

static int unset_gpio(struct adaptor_ctx *ctx, void *data, int val)
{
	unsigned long long idx;
	int ret;
	struct gpio_desc *gpio_desc;
	...
	if ((gpio_desc == g_gpio_desc->gpio[idx]) && (g_gpio_desc->gpio_pwr_cnt[idx] > 1)) {
		dev_dbg(ctx->dev, "other sensor use gpio now ,no need power off,gpio %s\n",
				 gpio_names[idx]);
		g_gpio_desc->gpio_pwr_cnt[idx]--;
		return 0;
	}
...
static int set_gpio_boolean(struct adaptor_ctx *ctx, void *data, int val)
{
	return set_gpio(ctx, data, !!val);
}
...
int do_hw_power_off(struct adaptor_ctx *ctx)
{
	...
	if (ctx->state[STATE_MIPI_SWITCH_ON] ||
		...
		ctx->state[STATE_RST_LOW] ||
		ctx->state[STATE_RST_HIGH]
		) {
...
int adaptor_hw_init(struct adaptor_ctx *ctx)
{
	...
	if (!g_gpio_desc)
		g_gpio_desc = devm_kzalloc(ctx->dev, sizeof(struct mtk_gpio_desc), GFP_KERNEL);
	if (!g_gpio_desc)
		return -ENOMEM;
	...
	/* gpio */
	for (i = 0; i < GPIO_MAXCNT; i++) {
		ctx->gpio[i] = gpiod_get_optional(
				dev, gpio_names[i], GPIOD_OUT_LOW);
		if (IS_ERR_OR_NULL(ctx->gpio[i])) {
			if (IS_ERR(ctx->gpio[i]) && (PTR_ERR(ctx->gpio[i]) == EBUSY)) {
				dev_dbg(dev, "error type[%d]\n", PTR_ERR(ctx->gpio[i]));
				ctx->gpio[i] = g_gpio_desc->gpio[i];
				dev_dbg(dev, "no gpio %s,get from g_gpio_desc\n", gpio_names[i]);
			} else {
				dev_dbg(dev, "no gpio %s\n", gpio_names[i]);
			}
		} else {
			g_gpio_desc->gpio[i] = ctx->gpio[i];
		}
	}
	...
	/* gpio */
	INST_OPS(ctx, gpio, GPIO_RST, HW_ID_RST,
			set_gpio, unset_gpio);

	INST_OPS(ctx, gpio, GPIO_PDN, HW_ID_PDN,
			set_gpio, unset_gpio);

	INST_OPS(ctx, gpio, GPIO_AVDD, HW_ID_AVDD,
			set_gpio_boolean, unset_gpio);

	INST_OPS(ctx, gpio, GPIO_DVDD, HW_ID_DVDD,
			set_gpio_boolean, unset_gpio);

	INST_OPS(ctx, gpio, GPIO_DOVDD, HW_ID_DOVDD,
			set_gpio_boolean, unset_gpio);

	INST_OPS(ctx, gpio, GPIO_AFVDD, HW_ID_AFVDD,
			set_gpio_boolean, unset_gpio);
