...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
		...
		.grabwindow_height = 1746, // record different mode's height of grabwindow
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 468000000,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[AR0430AP1302_SUPPORTED_SENSOR_MODE_NUMBER] = {
	...
	{2316, 1746, 0, 0, 2316, 1746, 2316, 1746, 0, 0, 2316, 1746, 0, 0, 2316, 1746}, /*hight speed video */
	{2316, 1746, 0, 0, 2316, 1746, 2316, 1746, 0, 0, 2316, 1746, 0, 0, 2316, 1746} /* slim video  */
};
...
static void update_client_info(struct subdrv_ctx *ctx, struct ar0430ap1302_device *ap1302_dev)
{
	ap1302_dev->dev = &ctx->i2c_client->dev;
	ap1302_dev->client = ctx->i2c_client;
	ap1302_dev->width_factor = AR0430AP1302_SLAVE_SENSOR_COUNT;
}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	struct ar0430ap1302_device ap1302_dev = {0};
	...
	ar0430ap1302_sensor_init(&ap1302_dev);
	// request firmware
	LOG_INFO("AR0430AP1302 request fw.");
	ret = ar0430ap1302_request_firmware(&ap1302_dev);
	if (ret) {
	...
	for (retries = 0; retries < 5; ++retries) {
		...
		ret = ar0430ap1302_load_firmware(&ap1302_dev);
		if (!ret)
			break;
		//ar0430ap1302_log_status(&ap1302_dev);
		ar0430ap1302_reset(&ap1302_dev);
...
static int close(struct subdrv_ctx *ctx)
{
	...
	//release_firmware(ap1302_dev.fw);
	ar0430ap1302_remove(&ap1302_dev);
	return ERROR_NONE;
} /* close  */
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	if (enable) {
		ar0430ap1302_stream(&ap1302_dev, enable);
