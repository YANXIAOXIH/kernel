...
static void update_client_info(struct subdrv_ctx *ctx, struct ar0830ap1302_device *ap1302_dev)
{
	ap1302_dev->dev = &ctx->i2c_client->dev;
	ap1302_dev->client = ctx->i2c_client;
	ap1302_dev->width_factor = AR0830AP1302_SLAVE_SENSOR_COUNT;
}
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	...
	struct ar0830ap1302_device ap1302_dev = {0};
	...
	ar0830ap1302_config_mode(&ap1302_dev, &info);
}   /*  preview_setting  */
...
static int open(struct subdrv_ctx *ctx)
{
	...
	ar0830ap1302_sensor_init(&ap1302_dev);
	// request firmware
	LOG_INFO("AR0830AP1302 request fw.");
	ret = ar0830ap1302_request_firmware(&ap1302_dev);
	if (ret) {
	...
	for (retries = 0; retries < 5; ++retries) {
		...
		ret = ar0830ap1302_load_firmware(&ap1302_dev);
		if (!ret)
			break;
		//ar0830ap1302_log_status(&ap1302_dev);
		ar0830ap1302_reset(&ap1302_dev);
...
static int close(struct subdrv_ctx *ctx)
{
	...
	//release_firmware(ap1302_dev.fw);
	ar0830ap1302_remove(&ap1302_dev);
	return ERROR_NONE;
} /* close  */
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	if (enable) {
		ar0830ap1302_stream(&ap1302_dev, enable);
