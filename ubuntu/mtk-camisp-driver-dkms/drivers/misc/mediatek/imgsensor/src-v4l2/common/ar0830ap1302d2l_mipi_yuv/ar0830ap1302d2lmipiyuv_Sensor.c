...
#include "ar0830ap1302d2lmipiyuv_Sensor.h"
...
#define PFX "AR0830AP1302D2L_camera_sensor"
...
#define AR0830AP1302D2L_SUPPORTED_SENSOR_MODE_NUMBER 5
#define AR0830AP1302D2L_LOG_STATUS_EN 0

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = AR0830AP1302D2L_CHIP_ID,
	...
	.sensor_mode_num = AR0830AP1302D2L_SUPPORTED_SENSOR_MODE_NUMBER,    // support sensor mode num
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[AR0830AP1302D2L_SUPPORTED_SENSOR_MODE_NUMBER] = {
...
static void update_client_info(struct subdrv_ctx *ctx, struct ar0830ap1302d2l_device *ap1302_dev)
{
	ap1302_dev->dev = &ctx->i2c_client->dev;
	ap1302_dev->client = ctx->i2c_client;
	ap1302_dev->width_factor = AR0830AP1302D2L_SLAVE_SENSOR_COUNT;
}

static void sensor_init(struct subdrv_ctx *ctx)
{
	LOG_INFO("AR0830AP1302D2L init()");
	// ar0830ap1302d2l_load_firmware
} /* sensor_init */

static void preview_setting(struct subdrv_ctx *ctx)
{
	struct ar0830ap1302d2l_mode_info info = {0};
	struct ar0830ap1302d2l_device ap1302_dev = {0};
	update_client_info(ctx, &ap1302_dev);
	...
	ar0830ap1302d2l_config_mode(&ap1302_dev, &info);
}   /*  preview_setting  */
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = read_cmos_sensor_16(ctx, AR0830AP1302D2L_REG_CHIP_ID);
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INFO("AR0830AP1302D2L i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_DBG("AR0830AP1302D2L Read sensor id fail, write id:0x%x id: 0x%x\n",
				ctx->i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		LOG_ERR("Get AR0830AP1302D2L sensor id fail.");
...
static int open(struct subdrv_ctx *ctx)
{
	...
	//sensor have two i2c address 0x6c 0x6d & 0x21 0x20,
	//we should detect the module used i2c address
	// LOG_INFO("AR0830AP1302D2L open().");
	if (get_imgsensor_id(ctx, &sensor_id) != ERROR_NONE) {
		LOG_ERR("AR0830AP1302D2L get_imgsensor_id() return error.");
		return ERROR_SENSOR_CONNECT_FAIL;
	}

	ar0830ap1302d2l_sensor_init(&ap1302_dev);
	// request firmware
	LOG_INFO("AR0830AP1302D2L request fw.");
	ret = ar0830ap1302d2l_request_firmware(&ap1302_dev);
	if (ret) {
		LOG_ERR("AR0830AP1302D2L Request Firmware Failed.");
		release_firmware(ap1302_dev.fw);
		return ret;
	}
	for (retries = 0; retries < 5; ++retries) {
		LOG_DBG("Try to load f/w. AR0830AP1302D2L LINE = %d.", __LINE__);
		ret = ar0830ap1302d2l_load_firmware(&ap1302_dev);
		if (!ret)
			break;
		//ar0830ap1302d2l_log_status(&ap1302_dev);
		ar0830ap1302d2l_reset(&ap1302_dev);
		LOG_DBG("Retry. AR0830AP1302D2L LINE = %d.", __LINE__);
	}
	if (retries == 5) {
	...
	sensor_init(ctx);
	LOG_DBG("SensorInit. AR0830AP1302D2L LINE = %d.", __LINE__);
	...
	release_firmware(ap1302_dev.fw);
	LOG_DBG("Open() Done. AR0830AP1302D2L LINE = %d.", __LINE__);
	return ERROR_NONE;
} /* open  */
...
static int close(struct subdrv_ctx *ctx)
{
	...
	//release_firmware(ap1302_dev.fw);
	ar0830ap1302d2l_remove(&ap1302_dev);
	return ERROR_NONE;
} /* close  */
...
static kal_uint32 streaming_control(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	if (enable) {
		ar0830ap1302d2l_stream(&ap1302_dev, enable);
#if AR0830AP1302D2L_LOG_STATUS_EN
		ar0830ap1302d2l_sensor_init(&ap1302_dev);
		ar0830ap1302d2l_log_status(&ap1302_dev);
		ar0830ap1302d2l_log_status(&ap1302_dev);
		ar0830ap1302d2l_log_status(&ap1302_dev);
...
static struct subdrv_pw_seq_entry pw_seq[] = {
	{HW_ID_PDN, 0, 0},
	{HW_ID_RST, 0, 0},
	{HW_ID_AVDD, 1800000, 0},
	{HW_ID_DVDD, 1800000, 0},
...
const struct subdrv_entry ar0830ap1302d2l_mipi_yuv_entry = {
	.name = "ar0830ap1302d2l_mipi_yuv",
	.id = AR0830AP1302D2L_SENSOR_ID,
	.pw_seq = pw_seq,
