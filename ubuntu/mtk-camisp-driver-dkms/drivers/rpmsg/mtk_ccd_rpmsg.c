...
void __ept_release(struct kref *kref)
{
	...
	dev_dbg(&mtk_subdev->pdev->dev, "free mtk rpmsg endpoint: %p\n",
		mept);
...
void mtk_rpmsg_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	if (ret)
		dev_dbg(&ept->rpdev->dev, "rpmsg handler return error = %d",
			ret);
}
...
static void mtk_rpmsg_destroy_ept(struct rpmsg_endpoint *ept)
{
	...
	while (atomic_read(&mept->ccd_cmd_sent) > 0) {
		dev_dbg(&mtk_subdev->pdev->dev, "%s: cmd_sent: %d\n",
			      ...
			      struct mtk_ccd_rpmsg_ops *ops)
{
	...
	dev_dbg(&pdev->dev, "%s mtk_ccd_msgdev addr: %d\n",
			 ...
			 int len, void *priv, u32 src)
{
	...
	dev_dbg(&mtk_subdev->pdev->dev, "%s: %d\n", __func__, src);
	...
	dev_dbg(&mtk_subdev->pdev->dev, "%s, src: %d, ept: %p\n",
		 __func__, src, ept);
...
static int ccd_msgdev_probe(struct rpmsg_device *rpmsg_device)
{
	...
	dev_dbg(&mdev->mtk_subdev->pdev->dev, "%s : %s\n", __func__,
		rpdrv->id_table->name);
	return 0;
}
