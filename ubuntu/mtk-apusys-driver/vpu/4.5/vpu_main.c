...
int vpu_set_init_info(struct mtk_apu *apu)
{
	...
	if (!apu || !apu->conf_buf) {
		pr_err("invalid argument: apu or conf_buf\n");
		return -EINVAL;
	}
...
int vpu_send_cmd_rt(int op, void *hnd, struct apusys_device *adev)
{
	...
	case APUSYS_CMD_EXECUTE:
		...
		vpu_cmd_debug("%s: vpu%d: EXECUTE, kva: %lx kid: 0x%llx subcmd_idx: 0x%x\n",
...
static int vpu_init_bin(struct platform_device *pdev,
			struct vpu_device *vpu_device)
{
	...
	if (of_property_read_u32(node, "bin-phy-addr", &phy_addr) ||
		...
		return vpu_loadimage(pdev, vpu_device);
	}
	...
	struct vpu_device *vd)
{
	...
	if (!vpu_drv->mva_algo && vd->id == 0) {
		...
		if ((vpu_drv->vpu_load_image == false) &&
			mops->dts(vd->dev, "algo", &vpu_drv->iova_algo))
			goto error;
...
static int vpu_init_dev_mem_rv(struct platform_device *pdev,
	struct vpu_device *vd)
{
	struct resource *res;
	dma_addr_t iova = 0;
	...
	if (!iova)
		goto error;
	iova = mops->alloc(dev, &vd->iova_kernel);
	if (!iova)
		goto error;
...
static int vpu_probe(struct platform_device *pdev)
{
	...
	if (vd->id >= 0 && vd->id < VPU_NUM_MAX)
		...
		pr_info("%s: invalid vpu core id: %d\n", __func__, vd->id);
		goto out;
	}
	...
	/* allocate resources */
	ret = probe_ops->init_dev_mem(pdev, vd);
	if (ret)
		goto free;
	...
	ret = probe_ops->init_dev_hw(pdev, vd);
	if (ret)
		goto free;
...
struct vpu_probe_ops vpu_probe_ops_ap = {
	...
	.init_dev_hw = vpu_init_dev_hw,
	.init_dev_mem = vpu_init_dev_mem,
};
...
struct vpu_probe_ops vpu_probe_ops_rv = {
	...
	.init_dev_hw = vpu_init_dev_hw_rv,
	.init_dev_mem = vpu_init_dev_mem_rv,
};
...
struct vpu_platform vpu_plat_mt8195 = {
	.bops = &vpu_bops_preload,
	.mops = &vpu_mops_v2,
	.sops = &vpu_sops_mt68xx,
	.cops = &vpu_cops_mt8195,
	.reg = &vpu_reg_mt68xx,
	.cfg = &vpu_cfg_mt8195,
	.cmd_ops = &vpu_cmd_ops_rv,
	.probe_ops = &vpu_probe_ops_rv,
...
static const struct of_device_id vpu_of_ids[] = {
	...
	{.compatible = "mediatek,mt8195-vpu_core0", .data = &vpu_plat_mt8195},
	{}
};
static const struct of_device_id vpu_of_ids1[] = {
	{.compatible = "mediatek,mt8195-vpu_core1", .data = &vpu_plat_mt8195},
	{}
};


MODULE_DEVICE_TABLE(of, vpu_of_ids);
MODULE_DEVICE_TABLE(of, vpu_of_ids1);
...
static struct platform_driver vpu_plat_drv1 = {
	.probe   = vpu_probe,
	.remove  = vpu_remove,
	.driver  = {
	.name = "vpu1",
	.owner = THIS_MODULE,
	.of_match_table = vpu_of_ids1,
	}
};
...
int vpu_init(struct apusys_core_info *info)
{
	...
	ret = platform_driver_register(&vpu_plat_drv1);
