...
static const char * const pll_name[] = {
				"PLL_CONN", "PLL_IOMMU", "PLL_VPU", "PLL_MDLA"};
static const char * const buck_name[] = {
				"BUCK_VAPU", "BUCK_VMDLA", "BUCK_VSRAM"};
...
static inline int over_range_check(int opp)
{
	...
	else if (opp > mt8195_opp_tbl->tbl_size - 1)
		return mt8195_opp_tbl->tbl_size - 1;
	else
		return opp;
}
				    ...
				    enum dvfs_device_id dvfs_dev_id)
{
	...
	if ((mt8195_opp_tbl == NULL) || (mt8195_opp_tbl->tbl_size == 0))
		return;
	...
	delta = mt8195_opp_tbl->opp[0].pll_freq[apu_clksrc_id] -
			mt8195_opp_tbl->opp[opp_min_idx].pll_freq[apu_clksrc_id];
	if (delta <= 0)
		delta = 1;

	for (i = 0; i <= opp_min_idx; i++) {
		step[i] = mt8195_opp_tbl->opp[i].pll_freq[apu_clksrc_id] -
			mt8195_opp_tbl->opp[opp_min_idx].pll_freq[apu_clksrc_id];
		if (step[i] > delta)
			step[i] = delta;
	}
...
static int aputop_show_opp_tbl(struct seq_file *s, void *unused)
{
	...
	for (i = 0 ; i < size ; i++) {
		...
		for (j = 0 ; j < PLL_NUM ; j++)
			seq_printf(s, "  %07d |",
				mt8195_opp_tbl->opp[i].pll_freq[j]);

		seq_puts(s, "\n");
	}

	seq_puts(s, "\n");
	for (i = DVFS_DEV_VPU; i < DVFS_DEV_MAX; i++)
...
int mt8195_apu_top_dbg_open(struct inode *inode, struct file *file)
{
	pr_info("%s ++\n", __func__);
...
#define MAX_ARG 4
ssize_t mt8195_apu_top_dbg_write(
...
int mt8195_apu_top_rpmsg_cb(int cmd, void *data, int len, void *priv, u32 src)
{
	...
	case APUTOP_DUMP_OPP_TBL:
		...
		total_tbl_size = sizeof(*mt8195_opp_tbl) +
		...
		if (mt8195_opp_tbl == NULL)
			mt8195_opp_tbl = kzalloc(sizeof(*mt8195_opp_tbl) + total_tbl_size,
					  GFP_KERNEL);
		else if (mt8195_opp_tbl->tbl_size < tbl_size) {
			pr_info("[%s][%d] mt8195_opp_tbl->tbl_size = %d, tbl_size = %d\n",
				__func__, __LINE__, mt8195_opp_tbl->tbl_size, tbl_size);
			tmp_opp_tbl = kzalloc(total_tbl_size, GFP_KERNEL);
			memcpy(tmp_opp_tbl, mt8195_opp_tbl, sizeof(*mt8195_opp_tbl));
			kfree(mt8195_opp_tbl);
			mt8195_opp_tbl = tmp_opp_tbl;
		}

		if (IS_ERR_OR_NULL(mt8195_opp_tbl))
			return -EINVAL;

		memcpy(mt8195_opp_tbl, data, total_tbl_size);
