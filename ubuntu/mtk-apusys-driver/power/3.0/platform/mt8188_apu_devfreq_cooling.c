				 ...
				 unsigned long freq)
{
	...
	for (opp = 0; opp < opp_tbl->tbl_size - 1; opp++) {
		next_freq = opp_tbl->opp[opp + 1].pll_freq[pll_idx] + 1;
		if (freq >= next_freq &&
			freq <= (opp_tbl->opp[opp].pll_freq[pll_idx] + 1))
			break;
	}
...
static int mt8188_apu_devfreq_set_target(struct device *dev,
...
static int mt8188_apu_devfreq_get_status(struct device *dev,
...
static int mt8188_apu_devfreq_get_cur_freq(struct device *dev,
					   unsigned long *freq)
{
	unsigned int dvfs_dev_id;
	struct apu_pwr_devfreq_st *apu_pwr_devfreq_ptr;
...
int mt8188_apu_devfreq_cooling_register(struct platform_device *pdev)
{
	...
	apu_pwr_devfreq_ptr->profile.target = mt8188_apu_devfreq_set_target;
	apu_pwr_devfreq_ptr->profile.get_dev_status =
						mt8188_apu_devfreq_get_status;
	apu_pwr_devfreq_ptr->profile.get_cur_freq =
						mt8188_apu_devfreq_get_cur_freq;
