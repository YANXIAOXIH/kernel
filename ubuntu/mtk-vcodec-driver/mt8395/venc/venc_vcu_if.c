				   ...
				   unsigned int *bs_size)
{
	...
	if (frm_buf) {
		...
		if (frm_buf->has_dynamic_ctl) {
			vsi->dynamicctrl_addr = frm_buf->dynamic_ctl_addr;
			vsi->dynamicctrl_size = frm_buf->dynamic_ctl_dma->size;
			vsi->dynamicctrl_fd = frm_buf->dynamic_ctl_fd;
		} else {
			vsi->dynamicctrl_addr = 0;
			vsi->dynamicctrl_size = 0;
		}
