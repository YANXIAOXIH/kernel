...
struct platform_device *vdec_pm_dev[MTK_VDEC_MAX_LARB_COUNT] = { NULL };
...
int mtk_vcodec_init_dec_pm(struct mtk_vcodec_dev *mtkdev)
{
	...
	mtk_v4l2_debug(0, "%s at: %d", __func__, __LINE__);
...
void mtk_vcodec_dec_pw_on(struct mtk_vcodec_pm *pm, int hw_id)
{
	...
	for (i = 0; i < MTK_VDEC_MAX_LARB_COUNT; i++) {
		if (vdec_pm_dev[i]) {
			ret = pm_runtime_get_sync(&vdec_pm_dev[i]->dev);
			if (ret < 0)
...
void mtk_vcodec_dec_clock_on(struct mtk_vcodec_pm *pm, int hw_id)
{
	...
	for (j = 0; j < clks_data->main_clks_len; j++) {
		...
		if (ret)
			mtk_v4l2_err("clk_prepare_enable id: %d, name: %s ret %d",
