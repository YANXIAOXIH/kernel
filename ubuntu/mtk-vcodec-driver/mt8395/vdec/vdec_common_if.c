...
static int vdec_init(struct mtk_vcodec_ctx *ctx, unsigned long *h_vdec)
{
	...
	*(ctx->ipi_blocked) = 0;
	...
	mtk_vcodec_debug(inst, "Decoder Instance >> %p, vsi_size:%d", inst, sizeof(struct vdec_vsi));
	...
	struct vdec_fb *fb, unsigned int *src_chg)
{
	...
	mtk_vcodec_debug(inst, "+ [%d] FB y_dma=%llx c_dma=%llx va=%p num_planes %d, input_driven:%d",
		inst->num_nalu, fb_dma[0], fb_dma[1], fb, num_planes, inst->vsi->input_driven);
	...
	if (bs == NULL) {
		if (fb == NULL)
			...
			return vcu_dec_reset(vcu,
				VDEC_DRAIN_EOS);   /* drain and return EOS frame (2) */
	}
	...
	inst->vsi->dec.vpeek = inst->ctx->dec_params.vpeek;
