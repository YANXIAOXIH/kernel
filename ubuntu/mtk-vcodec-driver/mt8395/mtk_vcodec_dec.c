...
static int *get_general_buffer_va(struct mtk_vcodec_ctx *ctx, int fd)
{
	int *va = 0;
	int i = 0;
	struct dma_buf *dmabuf = NULL;
	...
	struct mtk_vcodec_mem *current_bs)
{
	...
	/*
	 * m2m job was canceled when stream off
	 * we cancel put fb job for the next round
	 */
...
static void mtk_vdec_worker(struct work_struct *work)
{
	...
	ctx->dec_params.vpeek = src_buf_info->vpeek;
...
void mtk_vcodec_dec_empty_queues(struct file *file, struct mtk_vcodec_ctx *ctx)
{
	...
	while ((dst_vb2_v4l2 = v4l2_m2m_dst_buf_remove(ctx->m2m_ctx))) {
		...
		mtkbuf = container_of(dst_vb2_v4l2, struct mtk_video_dec_buf, vb);
		if (mtkbuf != NULL) {
			if (mtkbuf->frame_buffer.dma_general_buf != 0) {
				mtk_v4l2_debug(4,
					"[%d] general_user_fd = %d, buf = %p",
					ctx->id,
...
static int get_framerate_by_ts(struct mtk_vcodec_ctx *ctx)
{	int framerate = 0;
	u64 delta = 0;
	#define NS2MS(X) (X/(1000 * 1000))//ns

	if ((ctx->cur_ts <= ctx->last_ts) ||
		(ctx->cur_ts <= 0) ||
		(ctx->last_ts <= 0) ||
		((ctx->cur_ts - ctx->last_ts) < 8 * 1000 * 1000)) {
		if (ctx->frame_rate != 0)
	...
	delta = ctx->cur_ts - ctx->last_ts;
	framerate = 1000/NS2MS(delta);
	if (framerate <= 35)
		framerate = 30;
	else if (framerate <= 66)
		framerate = 60;
							...
							struct v4l2_buffer *buf)
{
	...
	/* reset flags to disable invalid features */
	...
	if (buf->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		ctx->input_max_ts =
			...
			timeval_to_ns(&buf->timestamp) : ctx->input_max_ts;
		ctx->last_ts = ctx->cur_ts;
		ctx->cur_ts = timeval_to_ns(&buf->timestamp);
		ctx->frame_rate = get_framerate_by_ts(ctx);
		...
		mtkbuf->vpeek = (int)buf->reserved2;
	} else {
...
static int vb2ops_vdec_buf_prepare(struct vb2_buffer *vb)
{
	...
	if (vb->vb2_queue->type != V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		if (mtkbuf->general_user_fd > -1) {
			mtkbuf->frame_buffer.general_buf_fd = mtkbuf->general_user_fd;
			mtkbuf->frame_buffer.dma_general_buf =
		...
		if (mtkbuf->meta_user_fd > 0) {
			mtkbuf->frame_buffer.meta_buf_fd = mtkbuf->meta_user_fd;
			mtkbuf->frame_buffer.dma_meta_buf =
	...
	if (vb->vb2_queue->memory == VB2_MEMORY_DMABUF &&
		...
		if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
			...
			mtk_v4l2_debug(4,
			   "[%d] Cache sync- TD for %llx sz=%d dev %p",
			   ctx->id,
...
#if IS_ENABLED(CONFIG_VB2_MEDIATEK_DMA_CONTIG)
	vdec_dma_contig_memops = mtk_dma_contig_memops;
	vdec_dma_contig_memops.map_dmabuf = vdec_dc_ion_map_dmabuf;
