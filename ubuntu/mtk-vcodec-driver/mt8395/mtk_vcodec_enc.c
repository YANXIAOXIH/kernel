...
void mtk_enc_put_buf(struct mtk_vcodec_ctx *ctx)
{
	...
	do {
		...
		if (rResult.frm_va != 0 && virt_addr_valid(rResult.frm_va)) {
			...
			if (rResult.is_last_slc == 1 && pfrm) {
				for (i = 0; i < pfrm->num_planes; i++) {
					mtk_v4l2_debug(1, "input dmabuf:%lx",
					(unsigned long)pfrm->fb_addr[i].dmabuf);
				}
			}
		}
...
static int vidioc_venc_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_MTK_ENCODE_TEMPORAL_LAYER_COUNT:
		mtk_v4l2_debug(2,
			"temporal_layer_pcount: %d, temporal_layer_bcount: %d\n",
			ctrl->p_new.p_u32[0], ctrl->p_new.p_u32[1]);
			  ...
			  struct mtk_vcodec_ctx *ctx)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		mtk_v4l2_debug(0, "AFBC size:%d superblock(%dx%d) superblock_count(%d)\n",
			...
			mtk_v4l2_debug(0, " bytesperline is set to :%d",
				pix_fmt_mp->plane_fmt[0].bytesperline);
			pix_fmt_mp->plane_fmt[0].sizeimage =
				(imagePixels * bitsPP / 8) +
			...
			mtk_v4l2_debug(0, " bytesperline is set to :%d %d",
				pix_fmt_mp->plane_fmt[0].bytesperline,
			...
			mtk_v4l2_debug(0, " bytesperline is set to :%d %d %d ",
				pix_fmt_mp->plane_fmt[0].bytesperline,
				pix_fmt_mp->plane_fmt[1].bytesperline,
		...
		mtk_v4l2_debug(0,
			       "w/h (%d, %d) -> (%d,%d), num_planes=%d sizeimage[%d,%d,%d] bytesperline[%d,%d,%d]",
			       org_w, org_h,
			       ...
			       struct venc_enc_param *param)
{
	...
	if (param->qp_control_mode == V4L2_MPEG_VIDEO_QP_CONTROL_MODE_FRAME)
		param->framelvl_qp = enc_params->framelvl_qp;
	param->wpp_mode = enc_params->wpp_mode;
	param->low_latency_mode = enc_params->low_latency_mode;
			    ...
			    struct v4l2_buffer *buf)
{
	...
	mtkbuf->frm_buf.has_dynamic_ctl = 0;
	mtkbuf->frm_buf.dynamic_ctl_dma = 0;
	...
	if (buf->flags & V4L2_BUF_FLAG_DYNAMIC_CTRL_META && buf->reserved2 > 0) {
		mtkbuf->frm_buf.dynamic_ctl_dma = dma_buf_get(buf->reserved2);
		if (IS_ERR(mtkbuf->frm_buf.dynamic_ctl_dma)) {
			mtk_v4l2_err("%s dynamic_ctl_dma is err 0x%p.\n", __func__,
				mtkbuf->frm_buf.dynamic_ctl_dma);

			mtk_venc_queue_error_event(ctx);
			return -EINVAL;
		}

		mtkbuf->frm_buf.dynamic_ctl_att =
			dma_buf_attach(mtkbuf->frm_buf.dynamic_ctl_dma, &ctx->dev->plat_dev->dev);
		mtkbuf->frm_buf.dynamic_ctl_sgt =
			dma_buf_map_attachment(mtkbuf->frm_buf.dynamic_ctl_att, DMA_TO_DEVICE);
		if (IS_ERR_OR_NULL(mtkbuf->frm_buf.dynamic_ctl_sgt)) {
			mtk_v4l2_err("dma_buf_map_attachment fail %d.\n",
				mtkbuf->frm_buf.dynamic_ctl_sgt);
			dma_buf_detach(mtkbuf->frm_buf.dynamic_ctl_dma,
				mtkbuf->frm_buf.dynamic_ctl_att);
			return -EINVAL;
		}
		mtkbuf->frm_buf.dynamic_ctl_addr =
			sg_dma_address(mtkbuf->frm_buf.dynamic_ctl_sgt->sgl);

		mtkbuf->frm_buf.has_dynamic_ctl = 1;
		mtkbuf->frm_buf.dynamic_ctl_fd = buf->reserved2;
		mtk_v4l2_debug(1, "[%d] Have dynamic fd, buf->index: %d, dynamic_dma: %p, addr: %llx, fd:%u",
			ctx->id, buf->index, mtkbuf->frm_buf.dynamic_ctl_dma,
			mtkbuf->frm_buf.dynamic_ctl_addr, buf->reserved2);
	}
...
static void vb2ops_venc_buf_finish(struct vb2_buffer *vb)
{
	...
	if (mtkbuf->frm_buf.dynamic_ctl_dma != 0) {
		mtk_v4l2_debug(1,
			"dma_buf_put dynamic_ctl_dma=%p, DMA=%llx",
			mtkbuf->frm_buf.dynamic_ctl_dma,
			(unsigned long long)mtkbuf->frm_buf.dynamic_ctl_addr);
		if (mtkbuf->frm_buf.dynamic_ctl_sgt != NULL) {
			dma_buf_unmap_attachment(mtkbuf->frm_buf.dynamic_ctl_att,
			mtkbuf->frm_buf.dynamic_ctl_sgt, DMA_TO_DEVICE);
			dma_buf_detach(mtkbuf->frm_buf.dynamic_ctl_dma,
				mtkbuf->frm_buf.dynamic_ctl_att);
		}
		dma_buf_put(mtkbuf->frm_buf.dynamic_ctl_dma);
		mtkbuf->frm_buf.dynamic_ctl_dma = 0;
	}
...
static int vb2ops_venc_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	mtk_v4l2_debug(0, "fourcc 0x%x, hier_ref_layer: %d, hier_ref_type: %d low_lat %d wpp %d, layer count:%d %d\n",
		ctx->q_data[MTK_Q_DATA_DST].fmt->fourcc,
		param.hier_ref_layer, param.hier_ref_type, param.low_latency_mode, param.wpp_mode,
		param.temporal_layer_pcount, param.temporal_layer_bcount);
...
int mtk_vcodec_enc_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	cfg.type = V4L2_CTRL_TYPE_U32;
	cfg.flags = V4L2_CTRL_FLAG_WRITE_ONLY;
	cfg.name = "Video encode vui sar description for Extended_SAR";
	cfg.min = 0x00000000;
	cfg.max = 0xffffffff;
	cfg.step = 1;
	cfg.def = 0;
	cfg.dims[0] = sizeof(struct mtk_venc_vui_info)/sizeof(u32);
	cfg.ops = ops;
	mtk_vcodec_enc_custom_ctrls_check(handler, &cfg, NULL);
	...
	cfg.type = V4L2_CTRL_TYPE_INTEGER;
	cfg.flags = V4L2_CTRL_FLAG_WRITE_ONLY;
	cfg.name = "encode max ltr frames";
	cfg.min = 0;
	cfg.max = 3;
	cfg.step = 1;
	cfg.def = 0;
...
#if IS_ENABLED(CONFIG_VB2_MEDIATEK_DMA_CONTIG)
	venc_dma_contig_memops = mtk_dma_contig_memops;
	venc_dma_contig_memops.map_dmabuf = venc_dc_ion_map_dmabuf;
...
void mtk_venc_lock(struct mtk_vcodec_ctx *ctx, u32 hw_id, bool sec)
{
