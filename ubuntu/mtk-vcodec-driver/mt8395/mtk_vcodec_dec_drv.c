...
static void mtk_vcodec_close_dvfs(struct mtk_vcodec_ctx *ctx)
{
	int hw_id = 0;
...
#if VDEC_POWER_CLOCK_IMPROVEMENT
	if ((dev->dec_cnt == 1) &&
		dev->vdec_power_always_on) {
		mtk_vcodec_close_dvfs(ctx);
		dev->vdec_power_always_on = FALSE;
	}
#endif
	/*
...
static int mtk_vcodec_dec_probe(struct platform_device *pdev)
{
	...
	dev->dec_cnt = 0;
#if VDEC_POWER_CLOCK_IMPROVEMENT
	for (i = 0; i < MAX_VDEC_HW; i++)
		atomic_set(&dev->atomic_dvfs_on[i], 0);
#endif
...
static int mtk_vdec_larb_probe(struct platform_device *pdev)
{
	...
	vdec_pm_dev[hw_id] = pdev;

	pm_runtime_enable(dev);
	mtk_v4l2_debug(0, "%s enable larb %d, hw_id %d\n", __func__, larb_id, hw_id);
	return ret;
}
static int mtk_vdec_larb_remove(struct platform_device *pdev)
{
	mtk_v4l2_debug(0, "%s disable larb\n", __func__);
	return 0;
}
 ...
 */
static int __init mtk_vcodec_driver_init(void)
{
	int ret;

	ret = platform_driver_register(&mtk_vcodec_dec_driver);
	if (ret) {
		pr_info("[VCODEC] mtk_vcodec_driver probe fail\n");
		return ret;
	}
	ret = platform_driver_register(&mtk_vdec_larb_driver);
	if (ret) {
		pr_info("[VCODEC] mtk_vdec_larb_driver probe fail\n");
		return ret;
	}
	return ret;
}

module_init(mtk_vcodec_driver_init);

/*
 * driver exit point
 */
static void __exit mtk_vcodec_driver_exit(void)
{
	platform_driver_unregister(&mtk_vcodec_dec_driver);
	platform_driver_unregister(&mtk_vdec_larb_driver);
}

module_exit(mtk_vcodec_driver_exit);
